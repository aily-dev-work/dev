...
from signals.models import WatchSource
    ...
    def handle(self, *args, **options):
        ...
        for source in sources:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"[DONE] {source.name}: entries={result['entries']} created={result['created']} matched={result['matched']} skipped={result['skipped']} discovered_products={result['discovered_products']} unmatched_products={result['unmatched_products']}"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"summary: created={total_created} matched={total_matched} skipped={total_skipped} discovered_products={total_discovered_products}"
            )
        )
