...
def calculate_technical_summary_5m(stock: WatchStock) -> TechnicalSummary:
    """
    指定した WatchStock について、5分足データからテクニカル指標を集計する。
    日足と同じ構造の TechnicalSummary を返す（MA5/25/75 は 5/25/75 本の終値平均、
    ...
    # 日足と同様 latest_date は日付文字列（YYYY-MM-DD）。5分足の「直近足の日付」
    latest_date_str = latest.datetime.date().isoformat()
