...
### 2.2 OpenAI 呼び出しフック: `_call_openai_with_package`
...
- デフォルト実装は `ImproperlyConfigured` を送出する。
- 実際に OpenAI と連携する場合は、この関数の中身を差し替える（例: 環境変数から API キーを読み込み、`analysis_package` と `user_note` を system/user メッセージとして渡し、JSON 文字列を返す）。
- テストではこの関数を monkeypatch してダミーの JSON を返すことで、外部通信なしで検証している。
